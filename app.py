    ...
    try:
        ...
        for i, (val, ci) in enumerate(zip(raw, conf)):
            ...
            forecast_list.append({
                ...
                "season": "A" if week_date.month in [9,10,11,12,1,2] else ("B" if week_date.month in [3,4,5,6,7,8] else "C"),
            })
        ...
        # Step 1: figure out the REAL next planting opportunity first
...
def reset_password():
    try:
        ...
        db  = get_db()
        cur = db.cursor()
        cur.execute("UPDATE users SET password=%s WHERE user_id=%s", (hashed, user["user_id"]))
        db.commit()
        rows = cur.rowcount
        cur.close()
        db.close()
...
def forgot_password():
    try:
        ...
        if not user:
            cur.close(); db.close()
            return jsonify({"status": "error", "message": "No account found with this phone number or email"}), 404
...
@app.route('/admin/apply_inflation', methods=['POST'])
def apply_inflation():
    """
